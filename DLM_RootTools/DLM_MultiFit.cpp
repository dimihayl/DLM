...
//printinfo=true;
    ...
    fitter.Config().SetMinimizer("Minuit","MigradImprove");
...
double DLM_MultiFit::operator() (const double *par){
    ...
    double * spec_par;
    for(unsigned int ispec=0; ispec<num_spectra; ispec++){
        spec_par = new double [FitFunction[ispec]->GetNpar()];
        for(int ipar=0; ipar<FitFunction[ispec]->GetNpar(); ipar++){
            spec_par[ipar] = par[ParMap[ispec][ipar]];
        }
        result += (*Chi2Fun[ispec])(spec_par);
        delete [] spec_par;
    }
    return result;
}
