...
DLM_Histo<complex<double>> ***Init_pXi_Haidenbauer2025(const char *InputFolder, CATS &Kitty, const int &TYPE, const int &CUTOFF)
{
    ...
    Kitty.SetNumChannels(NumChannels);
    for (unsigned short usCh = 0; usCh < NumChannels; usCh++){
        if(usCh==1) Kitty.SetNumPW(usCh, 3);
        else Kitty.SetNumPW(usCh, 1);
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++)
    {
        ...
        while (!feof(InFile))
        {
            ...
            {
                sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f",
                        &fRadius, &fDummy, &fReWf_LL, &fImWf_LL, &fReWf_X0n, &fImWf_X0n, &fReWf_Xp, &fImWf_Xp, &fReWf_LS, &fImWf_LS);

            }
            else if (uFile == 1)//3S1
            {
                sscanf(cdummy, "  %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f",
                        &fRadius, &fDummy, &fReWf_LL, &fImWf_LL, &fReWf_X0n, &fImWf_X0n, &fReWf_Xp, &fImWf_Xp, &fReWf_LS, &fImWf_LS, &fReWf_d_X0n, &fImWf_d_X0n, &fReWf_d_Xp, &fImWf_d_Xp, &fReWf_d_LS, &fImWf_d_LS);
            }
            else
            {
...
DLM_Histo<complex<double>>*** Init_pSigmaMinus_Haidenbauer(const char* InputFolder, CATS* Kitty, const int& TYPE){
    Init_pSigmaMinus_Haidenbauer(InputFolder,*Kitty,TYPE);
}
...
DLM_Histo<complex<double>>*** Init_pSigmaMinus_Haidenbauer(const char* InputFolder, CATS& Kitty, const int& TYPE){
    ...
    //2 spins (=> two el. channels)
    // + 2/3 inel. channels for S0/1
    const unsigned short NumChannels = 7;
    ...
    Kitty.SetChannelWeight(0,1./4.);//1S0 elastic
    Kitty.SetChannelWeight(1,3./4.);//3S1 elastic
    Kitty.SetChannelWeight(2,1./4.);//S0 Ln -> Sp
    Kitty.SetChannelWeight(3,1./4.);//S0 S0n -> Sp
    Kitty.SetChannelWeight(4,3./4.);//S1 Ln -> Sp
    Kitty.SetChannelWeight(5,3./4.);//S1 S0n -> Sp
    Kitty.SetChannelWeight(6,3./4.);//S1 Ln -> Sp (d-wave)
    ...
    Kitty.SetQuantumStatistics(false);
    const double Mass_p = 938.272;
    const double Mass_Sm = 1197.45;
    Kitty.SetRedMass((Mass_p*Mass_Sm)/(Mass_p+Mass_Sm));
    ...
    if(TYPE==0){
        char strCutOff[16];
        strcat(InputFileName[f1S0], "Sm11s0C.data");
        strcat(InputFileName[f3S1], "Sm13s1C.data");
    }

    else{
        printf("YOU BROKE SOMETHING in Init_pSigmaMinus_Haidenbauer\n");
        return NULL;
    }
    ...
    //Kitty.SetChannelWeight(6,3./4.);//S1 Ln -> Sp (d-wave)
            ...
            if(uFile==f1S0){
                sscanf(cdummy, " %f %f %f %f %f %f %f %f",&fRadius,&fDummy,&fReWf1,&fImWf1,&fReWf2,&fImWf2,&fReWf3,&fImWf3);
                //1S0
                HistoWF[0][0].SetBinContent(WhichBin,(fReWf1+fi*fImWf1)*fRadius);
                HistoPS[0][0].SetBinContent(WhichBin,0);

                //S0 Ln -> Sp
                HistoWF[2][0].SetBinContent(WhichBin,(fReWf2+fi*fImWf2)*fRadius);
                HistoPS[2][0].SetBinContent(WhichBin,0);

                //S0 S0n -> Sp
                HistoWF[3][0].SetBinContent(WhichBin,(fReWf3+fi*fImWf3)*fRadius);
                HistoPS[3][0].SetBinContent(WhichBin,0);
            }
            else if(uFile==f3S1){
                sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f",&fRadius,&fDummy,&fReWf1,&fImWf1,&fReWf2,&fImWf2,&fReWf3,&fImWf3,&fReWf4,&fImWf4);
                ...
                //S1 Ln -> Sp
                HistoWF[4][0].SetBinContent(WhichBin,(fReWf2+fi*fImWf2)*fRadius);
                HistoPS[4][0].SetBinContent(WhichBin,0);

                //S1 S0n -> Sp
                HistoWF[5][0].SetBinContent(WhichBin,(fReWf3+fi*fImWf3)*fRadius);
                HistoPS[5][0].SetBinContent(WhichBin,0);

                //S1 Ln -> Sp (d-wave)
                HistoWF[6][0].SetBinContent(WhichBin,(fReWf4+fi*fImWf4)*fRadius);
                HistoPS[6][0].SetBinContent(WhichBin,0);            
            }
            else{
                printf("WTF from Init_pSigmaPlus_Haidenbauer\n");
