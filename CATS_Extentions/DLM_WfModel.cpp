...
DLM_Histo<complex<double>> ***Init_pL_Haidenbauer2019(const char *InputFolder, CATS &Kitty, const int &TYPE, const int &CUTOFF)
{
    ...
    else if (CUTOFF == 601)
    {
        ...
        {
            // 1S0 ---------------------------------------------------------
            if (uFile == f1S0 && ABCD / 100 == 1)
            { // this is set differently in this case
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++)
    {
        ...
        while (!feof(InFile))
        {
            if (!fgets(cdummy, 511, InFile))
                continue;
            if (uFile == 0)
                sscanf(cdummy, "%s %f %s %s %f %s %f %s", buffer, &pLab, buffer, buffer, &PhaseShiftRe, buffer, &PhaseShiftIm, buffer);
            else
            {
...
// total wfs for L=0 (s-wave)
// ψ(r)=R(r). 0.5*1/sqrt(π)
